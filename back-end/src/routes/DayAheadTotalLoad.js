...
router.route('/:country/:freq/date/:date').get(async (req,res) => 
{
    ...
    const query ={
        text: `SELECT * from get_day_ahead_total_load($1 ,$2,cast($3 as INT),cast($4 as INT),cast($5 as INT),$6)`,
...
router.route('/:country/:freq/month/:date').get(async (req,res) => 
{
    const token = req.headers['x-observatory-auth'];
    const format = req.query.format;
    ...
    const query ={
        text: `SELECT * from get_day_ahead_total_load_month($1 ,$2,cast($3 as INT),cast($4 as INT),$5)`,
...
router.route('/:country/:freq/year/:date').get(async (req,res) => 
{
    const token = req.headers['x-observatory-auth'];
    const format = req.query.format;
    ...
    const query ={
        text: `SELECT * from get_day_ahead_total_load_year($1 ,$2,cast($3 as INT),$4)`,
